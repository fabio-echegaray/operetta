...
class ConfiguredChannels(Montage):
    ...
    def _save_render_row_col_fid(self, row, col, fid, path=None, max_width=50):
        ...
        out = np.zeros(images[0].shape + 3, dtype=np.int16)
    ...
    def _measure_row_col_fid(self, row, col, fid):
        ...
        cells = nuclei = nuclei_img = cells_mask = None
        ...
        for ch in cfg_ch:
            ...
            df = pd.DataFrame()
            for operation in ch['pipeline']:
                if operation == 'nucleus':
                    assert not _nuc, 'nucleus called twice in configuration'
                    _nuc = True
                    # --------------------
                    #     Find nuclei
                    # --------------------
                ...
                if operation == 'cell':
                    assert not _cll, 'cell called twice in configuration'
                    assert 'nucleus' in ch['pipeline'], 'cell processing needs nuclei data as a seed'
                    _cll = True
                    ...
                    for cl in cells:
                        ...
                        for nucleus in nuclei:
                            ...
                            if valid_sample:
                                ...
                                n_bum = affinity.scale(nucl_bnd, xfact=self.um_per_pix, yfact=self.um_per_pix,
                                                       origin=(0, 0, 0))
                                c_bum = affinity.scale(cell_bnd, xfact=self.um_per_pix, yfact=self.um_per_pix,
                                                       origin=(0, 0, 0))
                ...
                if operation == 'intensity_in_nucleus':
                    assert 'nucleus' in ch['pipeline'], 'intensity_in_nucleus processing needs nuclei data as a seed'
                    assert 'cell' in ch['pipeline'], 'intensity_in_nucleus processing needs cell data as a seed'
                    for cl in cells:
                        ...
                        for nucleus in nuclei:
                            ...
                            ix = df['id'] == cl['id']
                            df.loc[ix, '%s_int' % ch['tag']] = signal_int
                            df.loc[ix, '%s_dens' % ch['tag']] = signal_density

                if operation == 'particle_in_cytoplasm':
                    assert 'nucleus' in ch['pipeline'], 'intensity_in_nucleus processing needs nuclei data as a seed'
                    assert 'cell' in ch['pipeline'], 'intensity_in_nucleus processing needs cell data as a seed'
                    assert ch['name'] == 'centrosome', 'intensity_in_nucleus only accepts centrosome analysis currently'
                    for cl in cells:
